...
public class savefrequence : MonoBehaviour
{
     string chemin;
    ...
    public void Save()
    {
        ...
        for ( i=1 ; i<12 ; i++) 
        {
            if (mem.salle == "salle"+i)
            {
                ...
                if (val!=0)
                {
                    Struct freq= JsonUtility.FromJson<Struct>(jsonstring);

                    freq.frequencerespiratoire= val.ToString();

                    jsonstring = JsonUtility.ToJson(freq);
